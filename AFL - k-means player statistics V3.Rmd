...
As alluded to, we have a pre-existing grouping variable - players **ground position**. In Australian rules, there are four over-arching positions (midfielders, rucks, defenders & forwards). This provides some encouraging validation of the the Elbow & Silhouette Plots, which both indicated k = 4 clusters 
...
**[1]** Assigning player position based upon the most frequent part of the ground they are named on the team sheet

**[2]** Assigned player positions sourced from *Stats Insider*
...
Even so inspection of the mismatches does reveal some players for which the cluster algorithm would appear to have got *wrong* (at least wrong in so far as the clusters are a representation of ground-position!). For example, Tom Scully is not known for his forward craft, typically occupying the wings as a midfielder.
