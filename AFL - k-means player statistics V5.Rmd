...
<font size="4">This markdown provides an introduction to cluster analysis using principal components analysis & k-means. The goal is to evaluate if Australian Rules Football players field position can be determined by their match day statistics. Results of the cluster analysis will be evaluated against two different measures of field position - a [**ground truth**](https://datascience.stackexchange.com/questions/17839/what-is-ground-truth) to guide this analysis
...
R users are fortunate that a wealth of data from these websites is accessible via the **["FitzRoy"](https://cran.r-project.org/web/packages/fitzRoy/index.html)** package</font>

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r Load dependencies, results='hide', message=FALSE, warning=FALSE, include=FALSE}
# Data cleaning 
library(plyr)
library(snakecase)
library(tidyr)
...
```{r plot theme, results='hide', message=FALSE, warning=FALSE, include=FALSE}
plot_theme <-
...
To begin, lets preview the first six columns of the data:

```{r data load, results='hide', message=FALSE, warning=FALSE, echo=FALSE}
# player data .csv 
...
```{r print table preview, echo=FALSE, message=FALSE, warning=FALSE}
# Print table 
df.players %>% 
  select(Player_Name, player_team, Games, PositionType, kicks, handballs, disposals) %>%
  top_n(20) %>% 
  kable() %>% #head limits to just top rows
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive", font_size = 8))
```

There are 950 rows reflects a unique player record
...
```{r scale1, message=FALSE, warning=FALSE, echo=FALSE}
set.seed(123)
```
...
```{r PCA (4), message=FALSE, warning=FALSE, echo = FALSE}
fviz_eig(M1.pca)
```
...
```{r PCA (6), message=FALSE, warning=FALSE, echo=FALSE}
p1 <- 
...
```{r PCA (7), message=FALSE, warning=FALSE}
...
```{r k means (2), message=FALSE, warning=FALSE, echo=FALSE}
wss.df %>% 
...
```{r k means (5), message=FALSE, warning=FALSE, echo=FALSE}
sil.df %>% 
...
```{r k means (6), message=FALSE, warning=FALSE}
...
```{r k means plot (1), message=FALSE, warning=FALSE, echo=FALSE}
...
```{r k means plot (2A), message=FALSE, warning=FALSE, echo=FALSE}
...
```{r k means evaluate player, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r k means plot (2B), message=FALSE, warning=FALSE, echo=FALSE}
p2 <-
autoplot(M1.pca, 
         data = M1,
         colour = "Cluster",
...
```{r k means plot (3), message=FALSE, warning=FALSE, echo=FALSE}
# join cluster groupings back to data
...
```{r k means evaluate, message=FALSE, warning=FALSE, echo=FALSE}
# join players back
...
```{r k means evaluate1, message=FALSE, warning=FALSE, echo=FALSE}
# Change cluster names to ~ position
M1$Cluster[M1$Cluster == "2"] <- "Defender"
M1$Cluster[M1$Cluster == "4"] <- "Forward"
M1$Cluster[M1$Cluster == "1"] <- "Midfield"
M1$Cluster[M1$Cluster == "3"] <- "Ruck"
# look for mismatches 
...
```{r k means plot (4) pre-work, message=FALSE, warning=FALSE, echo=FALSE}
positions.SI <- read.csv("afl_stats_position_stats_insider.csv")
```

```{r print table preview SI, echo=FALSE, message=FALSE, warning=FALSE}
# Print table 
positions.SI  %>% 
  top_n(20) %>% 
  kable() %>% #head limits to just top rows
...
```{r k means plot (4) pre-work2, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r SI data mung 3, echo=FALSE}
# Change cluster grouping to ground truth 
...
```{r k means evaluate16, message=FALSE, warning=FALSE, echo=FALSE}
# look for mismatches 
M1$SI.Match <- M1$Cluster == M1$PositionSI

# M1$SI.Match[M1$SI.Match == "TRUE"] <- "MATCH"
# M1$SI.Match[M1$SI.Match == "FALSE"] <- "NO MATCH"
# table(M1['SI.Match'])

# M1$SI.Match <- ifelse(M1$Cluster == M1$PositionSI, "match", 
#                       ifelse(M1$Cluster != M1$PositionSI, "no match", "none"))

# Print table 
# M1 %>% 
#   select(Player, SI.Match, Cluster, PositionSI) %>%
#   kable() %>% #head limits to just top rows
#   kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive", font_size = 8))
...
~ **83%** of players cluster/position matched to stats insider field positions. Thus, a similar level of accuracy, with a relatively similar profile of misclassifications 

```{r k means evaluate18, message=FALSE, warning=FALSE}
...
Reviewing specific players to be classified incorrectly reveals an important difference in the nature of the data used for classification, & assigned field position. Stats Insider classifications were based upon players *current* position. For example, in the 2020 season, Ben McEvoy who had typically played as a ruckman was being trialed in a defensive role. Similarly, Jack Watts who had played most of his career forward was similarly being trialed in defense. Thus, rather than reflecting measurement error, the discrepant groupings reflect alternative definitions of field position (current versus historic)

```{r k means plot (4), message=FALSE, warning=FALSE, echo=FALSE}
# coerce to factor 
...
This guide provided an introductory guide to PCA & k-means. Based on statistical output alone, the model provides a reasonably accurate segmentation of players by their field position. Discrepancies between the ground truth & model output can mostly be explained by different assumptions to define field position (historic versus current playing position), & edge cases with ambiguous positions (i.e. utilities)