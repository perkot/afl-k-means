...
However, just dumping all the variables into the model & hoping for the best is not advisable. Including variables that are ineffective at grouping the data can have a *negative impact* on the results by obscuring useful variance

As such, identifying a satisfactory combination of variables is critical to the success of the cluster analysis. This is an iterative & patient process, that is greatly aided by *subject matter-expertise*. Even then, some aspect of trial-&-error is required, adding & subtracting variables, & documenting the subsequent effect on the output 

In the interest of brevity I won't labor on the various attempted models for this markdown. Suffice to say, the eventual model relied upon some intuition around what match-day statistics would likely discriminate players, then trial & error of different combinations
...
Below I have applied two of the most popular optimization methods for determining k, along with a discussion of our known grouping variable - player position
...
The already discussed, ace-in-the-hole for this dataset is that for AFL players, we have a pre-existing grouping variable - **their position on the ground**. There are four broad groupings of AFL positions (midfielders, rucks, defenders & forwards), made all the more interesting by the results of the Elbow & Silhouette Plots corroborating this number

- Add section here on fact ground truth not perfect

- two estimations
- (1) player positions from stats insider 
- (2) my own method of collating the position for which each player most frequently *starts* on the ground (excludes interchange)

Clearly all indicators suggest we proceed with a k of 4. As part of this analysis, we can compare the results of the k-means cluster to the already established ground positions of players
...
With 500 + data-points the plot is too dense to include every single player label. As such I have leveraged data from [stats insider](https://www.statsinsider.com.au/afl/player-ratings) that includes player ratings, to limit player labels to only the top handful of players in each position 
...
As such, 30 player labels are included 
...
autoplot(M1.pca, 
  ...
  theme_minimal() +
  plot_theme
p5$layers[[2]]$aes_params$size <- 0.1 # change line thickness 
p5
```

Casting ones attention on the player labels, you can see quite clearly that greater cluster separation occurs when limiting to star players. Presumably they are exemplars of their field-position, & thus provide a clearer statistical profile

### Plot 3 : Player position overlayed with PCA variable loadings

#### Starting Position 

In the below plot we are doing away with the k-means cluster results. In their place are groupings based upon *field position*, as defined by a players most commonly named starting position on the team sheet. 
...
This plot replicates plot 2, but replaces the k-means cluster groupings with the players actual position (aka our ground truth). It shows clearly the cluster results are in close accord with players ground position

- join player name back to our model data-set

- change cluster names from numeric to equivalent position 

- calculate column for mismatches 
...
- add a column depicting the top 5 players in each position
