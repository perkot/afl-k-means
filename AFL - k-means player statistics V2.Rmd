...
This markdown provides a gentle introduction to exploratory cluster analysis, via principal components analysis & k-means. The goal is to see if Australian Rules Football players can be *clustered* based upon their statistical output. The results of the cluster analysis will then be evaluated against a number of estimates of a [**ground truth**](https://datascience.stackexchange.com/questions/17839/what-is-ground-truth) - a players *field position*, a known way of grouping players
...
To begin, lets load in a .csv extract from this prior analysis 
...
Columns 1:3 provide basic demographics (players name, team, games played). Column four is the ground truth for this dataset: position

Columns five through 48 are *averaged* statistics (i.e. kicks, handballs, spoils, tacklers per game). These are the columns for which deeper analytics can be conducted

## Data preparation

There are two important steps I have taken prior to analysis 
...
If a dataset includes measures on different scales, or at a minimum has variables with different variances, it risks the analysis being in biased in favor of variables with *large* differences 
...
However, just dumping all the variables into the model & hoping for the best is not advisable. Including ineffective variables can have a *negative impact* on the results by obscuring useful variance

As such, identifying a satisfactory combination of variables is critical to success. Clustering can be an iterative & patient process, that is greatly aided by *subject matter-expertise*. Even then, some degree of trial-&-error is required ... adding & subtracting variables, & documenting the subsequent effect on the output 

In the interest of brevity I won't labor on numerous failed models for this markdown. Suffice to say, the eventual model relied upon some intuition around what match-day statistics would likely discriminate players, then trial & error of different combinations

Some metrics were especially conspicuous (i.e. *hitouts* as something likely to distinguish players), whilst others were more ambiguous (*uncontested possessions* seemed less clear in its discriminatory powers)

The 15 statistics I settled upon to include in the model are summarized below
...
Although some information will be lost in compression, the idea is that a small degradation in accuracy is traded-off for an increase in parsimony. Kind of the statistical equivalent of a compressed MP3 retaining much of the quality of an uncompressed WAV file, at a fraction of the file-size
...
PCA does have one very obvious benefit to cluster analysis - it allows for simpler visualization of results. It is nigh on impossible to provide a simple summary visualization of higher-dimensional data. As such, dimensional reduction allows for more information from the analysis to be visualized in a two-dimensional format
...
Further, we can visualize **all** of the variable loadings for the *first two* principal components. Each point on the plot reflects an individual player. The x-axis depicts PC1, whislt the y-axis depicts PC2
...
**nstart = 25** : The number of times the initial starting points are re-sampled.
It looks for the initial starting points that have the lowest WSS
...
### Plot 2A : Clusters overlayed with PCA variable loadings
...
### Plot 2B : Clusters overlayed with PCA variable loadings & player labels
...
As alluded to, for AFL players, we have a pre-existing grouping variable - their **ground position**. In Australian rules, there are four over-arching ground positions (midfielders, rucks, defenders & forwards). This is encouraging validation for the current model, when considering the Elbow & Silhouette Plots corroborated four groups

Despite player position being an established grouping variable for players, exactly how it is measured / quantified is a little more complex. Some of these complicating factors include

* the fact players can play in multiple positions 
* players can change position over the course of their career 
* positions can be more granular (i.e. key forwards versus small forwards)

So although the ground truth provides a compass of sorts for model evaluation, it shouldn't necessarily be considered an immutable truth, for all circumstances, or observations 

We will proceed with two measures of a players ground position 

Firstly, an aggregated view of player positions from *Stats Insider*

Second, my own method of ascribing a players position as the position for which they are most frequently named on the team-sheet 

### Plot 3 : Player position overlayed with PCA variable loadings

#### Starting Position 

In the below plot we are exchanging the k-means cluster results with the field position groupings, as determined by named position 
...
On visual inspection, the plot shows pretty close concordance with the cluster results. Two stand-out differences between cluster & ground-truth are that

* midfielders look to *bleed* into the defender group
* midfielders look to *bleed* into the forward group 

Using tidyverse, we can quantify exactly how many players cluster group matched their most comomnly named starting position:
...
The majority of mismatches (67%) were players whose starting position was as a midfielder, but the cluster defined as either a defender or forward

Looking at these on a case-by-case basis, many of these players position is  ambiguous to determine. For example, the cluster algorithm designated Dustin Martin as a midfielder, while the *ground truth*, based on his named position in the team was as a forward. This edge case is perhaps revealing of a limitation of this analysis, in that beholding players positions to one of four (midfield, defender, forward, ruck) may be too limiting.  Dustin Martin may be better captured as a *utility*, or a hybrid-position such as *midfield-forward*

Having said that, the mismatches reveal some players for which the cluster algorithm would appear to have got *wrong* (or at least wrong, in so far as the clusters are a representation of ground-position!). For example, Tom Scully is not known for his forward craft, typically occupying the wings as a midfielder.

### Plot 4 : Player position (stats insider) overlayed with PCA variable loadings
