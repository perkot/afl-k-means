...
In this markdown I will provide a gentle introduction to exploratory cluster analysis, via principal components analysis & k-means. The goal is to see if AFL players can be grouped, or *clustered* based upon their statistical output. These results will then be evaluated against a **ground truth** - a players *field position*, which is a known, real-world way of grouping players
...
### Plot 2 : Clusters overlayed with PCA variable loadings

```{r k means plot (2), message=FALSE, warning=FALSE}
...
Above is a somewhat experimental plot, combining components of the PCA & k-means analysis. The colour of each data-point represents the k-means cluster it has been assigned to, whilst the gray lines represent the variable loadings for the first two principal components. This shows fairly clearly that each of the four clusters align pretty closely with metrics one would expect to be associated with a players field position 
