...
I have curated my own data that summarizes *average game-day statistics* of Australian Rules Football players (i.e. average kicks, handballs, tackles per game)
...
Something crucial for k-means is that inclusion of *redundant* variables in the model (i.e. a metric that is ineffective at grouping the data) can have a *negative impact* on the eventual results by obscuring useful variance

As such, identifying a satisfactory combination of variables is critical to the eventual success of the cluster analysis. This is an iterative & patient process, that is greatly aided by subject matter-expertise. Even then, some aspect of trial-&-error is required, adding & subtracting variables, & documenting the subsequent effect on the output 
...
Some metrics were especially conspicuous (i.e. *hitouts* as something likely to distinguish players), whilst others were more ambiguous (*uncontested possessions* seemed less clearly in its discriminatory powers)
...
With our data finalized we can jump into the first-step for this cluster analysis: **principal components analysis**
...
Although some information will be lost in compression, the idea is that a small degradation in accuracy is traded-off for an increase in parsimony (kind of the statistical equivalent of a compressed MP3 compared to an uncompressed WAV file)
...
Note here that covariance is distinct from correlation - two related, but different metrics
...
Here we reach the punchline of PCA, & the value of dimension-reduction. With just three principal components, 85% of variance from our original model of 15 match-day statistics can be explained
...
k-means is a fast & powerful analysis, but comes with caveats 

[1] k-means will partition data, even if no *real* clusters exist. Real is a loose term here, but the point is that even if a data-set amounts to random noise, the k-means algorithm *will not fail*. Clusters will be generated even if they are meaningless. Fortunately there are ways to evaluate *cluster quality*

[2] k-means generates ~ spherical clusters. As such, visual inspection of k-means results is important. As emphasized by this [awesome analysis by David Robinson](http://varianceexplained.org/r/kmeans-free-lunch/), k-means can successfully reduce WSS, but miss obvious clusters in the dataset should they not be spherical. As such it is horses for courses - finding the right clustering algorithm that is able to service the assumptions of the dataset
...
Firstly, that k = 4 is very clearly the optimal division of clusters for this dataset. It's nice that the Silhouette & Elbow plots both yield the same suggested k, adding validity to this cluster-number 
...
This is on the surface disappointing, but again, needs to be interpreted within the context of what we know about this data. We can concede from this that at a minimum, the clustering results are not strong. But this isn't wholly surprising when considering the highly dynamic nature of AFL football, & immense diversity of athletes & outputs of players.  

However, to outright dismiss them as *artificial* would be premature. This is where subject matter expertise becomes important to the analysis, as we do know that there exist real-life groupings for players

# https://stats.stackexchange.com/questions/10540/how-to-interpret-mean-of-silhouette-plot

### Ground Truth 

The ace-in-the-hole for this dataset is that for AFL players, we have a pre-existing grouping variable - **their position on the ground**. Whats more interesting is in the broadest terms, player positions can be divided into four - midfielders, ruckmen, defenders & forwards

Although it is entirely possible that game day statistics could produce different groupings to ground position, it is interesting our optimization methods are in concert with this ground truth
...
Running the analysis produces a number of outputs 

1) Cluster means for each cluster (k), & each principal component included in the analysis 

2) Cluster assignment (either 1, 2, 3 or 4) for each included player

3) Statistics related to variance explained. **Total sum of squares** is the sum of squared distances of each observation to the overall sample mean. In addition, sample means are calculated per cluster. The sum of squared distances of these cluster means (in this case 4) to the overall mean is **between group sum of squares** 

The **78.5%** represents the total percentage of variance explained by the clusters. Generally a larger percentage is favorable, but it is also directly related to how many clusters are included. If there were as many clusters as there were observations, then this value would be 100%, but this would be completely self-defeating to the purpose of clustering

Let's now visualize how the clustering looks
...
ggplot(data, aes(x, y)) + 
  ...
  geom_point(alpha = 0.01) +
    labs(title = "Cluster Plot",
       subtitle = "Depicting of k = 4 for first two principal components") +
  theme(legend.position = "right",
        legend.direction = "vertical",
        legend.title = element_text(colour = "#4E4F4E",
                                    size = 8,
...
colour_theme3 <- c("#E4796A", "#E7B573", "#B0AA96", "#638DCB")
```
...
ggplot(data, aes(x, y)) + 
  ...
  geom_polygon(data = hull_data, 
               alpha = 0.5, 
               aes(fill = cluster, linetype = cluster)) +
  scale_color_manual(values = colour_theme3) +
  scale_fill_manual(values = colour_theme3) +
